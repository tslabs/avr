...
;			out ADMUX,a
;			ldi a,((1<<ADEN)|(1<<ADSC)|(1<<ADFR)|(0<<ADIE)|(1<<ADPS2)|(1<<ADPS1)|(1<<ADPS1))
;			out ADCSRA,a
...
QT1:
			...
			ldi yl,low(IVT0)
			ldi yh,high(IVT0)
			rcall CHAN			;Process one channel per quant
			ldi yl,low(IVT1)
			ldi yh,high(IVT1)
			rcall CHAN
			ldi yl,low(IVT2)
...
SONG_INIT:	;Module Initialisation
			...
			;Set TICK=1 so the first QUANT cycle will rcall NOTE procedure
			ldi a,1
			sts TICK,a
...
L2:			
			...
			breq L3
			sts IVT@0+PSM,c			;if inc of real address, store PSM
;			mov b,c					;skip interpolation - HERE is a GREAT BUG on some modules
L3:
			clr r1
			adc zh,r1
if_xm		adc r2,r1
