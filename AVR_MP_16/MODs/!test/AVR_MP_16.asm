...
MAIN:
			...
			ldi a,0x80			;Neutral values for PWM
			out L_OCR,a
			out R_OCR,a
...
CN4:			
			...
			;Slide up/down processing		(NO CHECK for overflow is done!!!!!)
			ldd al,Y+PER
			ldd ah,Y+PER+1
			...
			add al,a			;If slide positive (i.e. Down)
			adc ah,b
			rjmp CN2
...
CN3:		;VolumeSlide processing
			lds a,TICK
			lds r0,SPEED
			cp a,r0
			breq CN7			;If 1st tick in row, then skip VS
...
NUL_LP:		;Nulling Loop Row's and Loop Counters
			clr r0
			sts IVT0+LP_R,r0
			sts IVT0+LP_C,r0
			sts IVT1+LP_R,r0
...
NOTE:		;Single channel note processing
			...
			clr a
			std Y+SLIDE,a		;Set SLIDE = 0
			std Y+APG,a			;Nulling APG
			std Y+APG_F,a
			std Y+V_SL,a		;Nulling VolumeSlide
			...
			mov r2,b			;Keep original sample number
			...
			or a,bh
			brne NT2			
								;Note period is unspecified - so note is not processed
			com r7
			lds a,SUF
			and a,r7
...
NT3:
			std Y+SAMP,b		;Store current sample
...
_Exx:							;Extended effects processing
			...
			subi zl,-EFE_VEC
			clr zh
			andi ch,0x0F		;Mask effect parameter with 0x0F
			ijmp
...
_Fxx:							;Set Speed
			...
			cpi ch,0x21
			brcs EF1
								;If xx>0x20 then set TEMPO (OpenMPT behavior)
			sts TEMPO,ch
			mov r5,ch
EF2:		clr r6
			ldi a,low(p_qnt)
			...
			sts N_QNT+1,r2
			ret
EF1:							;If param<0x21 then set SPEED (OpenMPT behavior)
			sts SPEED,ch
EF3:
			ret
...
if_xm		elpm c,Z+				;read sample_1
if_no_xm	lpm c,Z+
			subi c,128				;make it unsigned
...
if_no_xm	lpm c,Z
			sbiw zl,1				;restore Z after Z+ read
			subi c,128
			...
			lds c,IVT@0+S_SAD		;inc sample sub-address
			lds r1,IVT@0+S_INC
			add c,r1
			sts IVT@0+S_SAD,c
			...
			lds c,IVT@0+S_INC+2		;inc sample address
			adc zl,c
			clr c
			adc zh,c
if_xm		adc r2,c
...
SONG_INIT:	;Module Initialisation
			...
			;Set S_NOP for sample variables
			ldi yl,low(IVT0+S_AD)
			ldi yh,high(IVT0+S_AD)
			call SI8
			call SI8_
			ldi yl,low(IVT1+S_AD)
			ldi yh,high(IVT1+S_AD)
			call SI8
			call SI8_
			ldi yl,low(IVT2+S_AD)
			ldi yh,high(IVT2+S_AD)
			call SI8
			call SI8_
			ldi yl,low(IVT3+S_AD)
			ldi yh,high(IVT3+S_AD)
			call SI8
			call SI8_
...
SI3:
			lpm a,Z
			andi a,0x0F				;Mask for Finetune value (fool-proof protection)
			st X+,a
			adiw zl,MST_SIZE
			dec i
			brne SI3
